...
#		print "flow $len\n";
		...
		if($nflag != 0) {
			...
			$nat = "$nflag,$ip_snat,$ip_dnat,$sport_nat,$dport_nat";
		}
		if($proto_app > 0) {
		...
		print "$tm1 $tm2 4 $np $ip_s $ip_d $sport $dport $cpi,$cpo,$cbi,$cbo $ifidx,$ofidx $proto_app$proto_master $nat$nhost$ncert\n";
	} elsif(($rt & 0x3) == 3) {
		my $len = 48-4;
		$n = sysread(O,$buf,$len);
		...
		my ($tm1,$cpi,$cpo,$cbi,$cbo) =
			unpack 'IIIQQ',$buf;
		print "LOST TRAFFIC $cpi,$cpo,$cbi,$cbo\n";
	} else {
